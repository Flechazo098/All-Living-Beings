    ...
    public static void handle(RequestDimensionsPacket pkt, Supplier<NetworkEvent.Context> ctxSupplier) {
        ...
        ctx.enqueueWork(() -> {
            ...
            if (!Util.isOwnerActive(sp)) {
                sp.displayClientMessage(Component.translatable("message.all_living_beings.not_god"), true);
                return;
            }
            ...
            NetworkHandler.CHANNEL.send(PacketDistributor.PLAYER.with(() -> sp), new SyncDimensionsPacket(ids));
        });
